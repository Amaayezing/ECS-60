...
class LLRB_multimap {
 public:
  ...
  const K& Min();
  // Insert @key in tree
  void Insert(const K &key, const std::vector<V> &values);
  // Remove @key from tree
  void Remove(const K &key);
  // Print tree in-order
  void Print();
  // Receives key and returns value associated to specific key
...
LLRB_multimap<K, V>::Get(LLRB_multimap<K, V>::Node* n,
                                                   const K &key) {
  while (n) {
    if (key == n->key)
      return n;
...
template <typename K, typename V>
void LLRB_multimap<K, V>::Insert(const K &key,
                                 const std::vector<V> &values) {
  Insert(root, key, values);
  cur_size++;
  root->color = BLACK;
}
...
void LLRB_multimap<K, V>::Insert(std::unique_ptr<Node> &n,
                                 const K &key, const std::vector<V> &values) {
  if (!n)
    n = std::unique_ptr<Node>(new Node{key, values, RED});
  else if (key < n->key)
    Insert(n->left, key, values);
  else if (key > n->key)
    Insert(n->right, key, values);
  else
    n->values.insert(n->values.end(), values.begin(), values.end());
...
const V& LLRB_multimap<K, V>::Get(const K& key) {
  ...
  while (n) {
    ...
    if (key < n->key) {
      if (Get(n->left.get(), key)) {
        return Get(n->left.get(), key)->values.front();
      }
    } else if (key > n->key) {
      if (Get(n->right.get(), key)) {
        return Get(n->right.get(), key)->values.front();
      }
    } else {
