...
void IndexMinPQ<K>::Push(const K &key, unsigned int idx) {
  ...
  cur_size++;
  heap_to_idx.at(cur_size) = idx;
  idx_to_heap.at(idx) = cur_size;
  keys.at(idx) = key;
...
void IndexMinPQ<K>::Pop() {
  ...
  unsigned int lastItem = heap_to_idx.at(cur_size);
  heap_to_idx.at(Root()) = lastItem;
  heap_to_idx.at(cur_size) = 0;
  cur_size--;
  PercolateDown(Root());
  idx_to_heap.at(lastItem) = 0;
  CheckHeapOrder(Root());
...
void IndexMinPQ<K>::ChangeKey(const K &key, unsigned int idx) {
  ...
  unsigned int priorKeyVal = keys.at(idx);
  unsigned int treePos = 0;

  keys.at(idx) = key;
  for (unsigned int i = 1; i <= cur_size; i++) {
    if (idx == heap_to_idx.at(i)) {
      treePos = i;
      break;
    }
  }
  if (priorKeyVal < key) {
    PercolateDown(treePos);
  } else if (priorKeyVal > key) {
    PercolateUp(treePos);
  }
